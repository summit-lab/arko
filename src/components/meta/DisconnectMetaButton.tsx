...
export function DisconnectMetaButton({ workspaceId }: DisconnectMetaButtonProps) {
  ...
  async function handleDisconnect() {
    const confirmed = window.confirm("¿Querés desconectar esta cuenta de Instagram? Vas a poder volver a conectarla cuando quieras.");
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(json?.message || "No pudimos desconectar la cuenta.");
      }

      router.refresh();
    } catch (disconnectError) {
      setError(disconnectError instanceof Error ? disconnectError.message : "No pudimos desconectar la cuenta.");
    } finally {
      setLoading(false);
    }
  }

      ...
      {isConnected ? (
        <div className="glass-panel rounded-xl p-8 border border-emerald-500/20">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">Cuenta conectada</h2>
          ...
          <Link
            href="/instagram"
            className="inline-flex items-center gap-2 bg-white/10 hover:bg-white/15 border border-white/10 text-sm text-white px-5 py-2.5 rounded-lg transition-colors mt-5"
          >
          ...
          <div className="glass-panel rounded-xl p-8 border border-white/10">
            <div className="flex items-start gap-5">
              ...
              <div className="flex-1">
                <h2 className="text-lg font-semibold text-white">Conectar Instagram Business</h2>
                <p className="text-sm text-zinc-400 mt-1">
                  Usamos Meta OAuth para conectarnos de forma segura. Solo necesitamos permisos de lectura.
                </p>
                {connectionStatus && connectionStatus !== "active" ? (
                  <p className="text-xs text-zinc-500 mt-2">La conexión actual no está activa. Podés reiniciar el login desde acá.</p>
                ) : null}
            ...
            <div className="grid grid-cols-2 gap-3">
              {PERMISSIONS.map((p) => (
                <div key={p.desc} className="flex items-center gap-3 p-3 rounded-lg bg-white/[0.03]">
                  <p.icon className="h-4 w-4 text-zinc-500 shrink-0" />

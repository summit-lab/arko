...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    { label: t("stats.clients"), value: profilesRes.count ?? 0, icon: Users, color: "text-blue-400" },
    { label: t("stats.workspaces"), value: workspacesRes.count ?? 0, icon: Building2, color: "text-violet-400" },
    { label: t("stats.activeConnections"), value: connectionsRes.count ?? 0, icon: Wifi, color: "text-emerald-400" },
    { label: t("stats.pendingInvitations"), value: invitationsRes.count ?? 0, icon: UserPlus, color: "text-amber-400" },
  ];
      ...
      <div className="glass-panel rounded-xl p-6 animate-slide-up stagger-5">
        ...
        <div className="space-y-1">
          <div className="grid grid-cols-12 gap-2 text-[10px] text-white/30 uppercase tracking-[0.1em] font-medium pb-3 border-b border-white/[0.06] px-2">
            <div className="col-span-4">{t("recent.headerName")}</div>
            <div className="col-span-4">{t("recent.headerEmail")}</div>
            <div className="col-span-2 text-center">{t("recent.headerRole")}</div>
            <div className="col-span-2 text-right">{t("recent.headerDate")}</div>
          </div>
          {recentUsers.length === 0 && (
            <p className="text-white/25 text-[13px] py-4 text-center">{t("recent.empty")}</p>
          )}
          {recentUsers.map((u) => (
            <div key={u.id} className="grid grid-cols-12 gap-2 items-center py-3 rounded-lg hover:bg-white/[0.03] transition-all duration-200 px-2">
              ...
              <div className="col-span-2 text-right text-[12px] text-white/30">
                {new Date(u.created_at).toLocaleDateString(dateLocale, { day: "2-digit", month: "short" })}
              </div>
            </div>
          ))}
        </div>
